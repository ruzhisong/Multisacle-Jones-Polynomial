...
def jones_ca_int_2(pdbid, filepath, min_distance, max_distance, times):
    coords, label = get_protein_ca_atom_coordinate(pdbid, filepath)
    #print(len(coords))
...
def fitting_1( X_norm, y, model):
    regressor = model.fit(X_norm, y)
    y_pred = regressor.predict(X_norm)
    return pearsonr(y_pred, y)[0], y_pred
...
def average_pvalue_of_data_set(data_set):
    ...
    for pdb in range(len(pdb_id)):
        ...
        model = Lasso()
        pvalue = fitting(x_norm_1, label, model)
    ...
    return mean_value, average

#generate_feature('b_factor_data_small/2olx.pdb')
def _main_ ():
    pdb_text = open('small_ID.txt')
    pro_pdb_id = pdb_text.readlines()
    pdb_id = eval(pro_pdb_id[0])
    pdb_text.close()
    ...
    for pdb in range(len(pdb_id)):
        ...
        print(pdb)
        print(pdb_id[pdb])
        generate_feature('b_factor_data_small/'+ pdb_id[pdb] +'.pdb')

#execution_time = timeit.timeit(_main_, number=1)
#print(f"运行时间: {execution_time} 秒")
#coords, label = get_protein_ca_atom_coordinate('pdbid', '2gr8' + '.pdb')

#average = average_pvalue_of_data_set('large')
...
def process_pdb_1(filepath):
    folder_name = os.path.splitext(filepath)[0]  # 使用文件名（不含扩展名）作为文件夹名
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    
    radius_range = range(20, 69)
    times = 200
    '''
    for r in radius_range:
        r_ = r / 4
        print("Processing radius:", r_)
        
        # gauss_link_ca_ = gauss_link('pdbid', filepath, r_ - 1, r_, 50)
        feature_ca, jones_ca = jones_ca_int('pdbid', filepath, r_ - 1, r_, times)
        # phe_ca = far_phe('pdbid', filepath, r_ - 1, r_, 50)  
        
        # np.save(os.path.join(folder_name, 'gauss_link_' + str(r_ - 1) + '_' + str(r_) + '.npy'), gauss_link_ca_)
        np.save(os.path.join(folder_name, 'jones_feature_'+str(times)+'_' + str(r_ - 1) + '_' + str(r_) + '.npy'), feature_ca)
        np.save(os.path.join(folder_name, 'jones_polynomial_' + str(r_ - 1) + '_' + str(r_) + '.npy'), jones_ca)
        # np.save(os.path.join(folder_name, 'fri_' + str(r_ - 1) + '_' + str(r_) + '.npy'), phe_ca)
    '''
    coords, label = get_protein_ca_atom_coordinate('pdbid', folder_name + '.pdb')
    ...
    for r in radius_range:
        r_ = r / 4
        print("Loading radius:", r_)  
        
        jones = np.load(os.path.join(folder_name, 'jones_feature_'+str(times)+'_' + str(r_ - 1) + '_' + str(r_) + '.npy'))
        pro_jones_feature.append(jones)
        # fri = np.load(os.path.join(folder_name, 'fri_' + str(r_ - 1) + '_' + str(r_) + '.npy'))
        # pro_fri_feature.append(fri)
        # gauss = np.load(os.path.join(folder_name, 'gauss_link_' + str(r_ - 1) + '_' + str(r_) + '.npy'))
    ...
    model = Lasso()
    prov, y_pored = fitting_1(x_norm_1, label, model)
    ...
    plt.plot(data1, label='JP', marker='x')
    plt.plot(data2, label='Experimental', marker='o')

    # 添加标题和标签
    plt.title('B-factor predictions for protein 1V70')
    plt.xlabel('Residue Number')
    plt.ylabel('B-factor')
    plt.legend()

    # 保存图像
    plt.savefig('B_facter_'+str(times)+'_'+str(radius_range)+'_'+filepath +'.png', dpi=300, bbox_inches='tight')
    # 显示图表
    plt.show()
    return prov
...
def process_pdb(filepath):
    ...
    # 保存 y_pored 为 txt 文件
    with open('1v70_y_pored.txt', 'w') as f:
        for item in y_pored:
            f.write("%s\n" % item)
    ...
    plt.plot(data1, label='mJP', marker='x')
    plt.plot(data2, label='Experimental', marker='o')

    # 添加标题和标签
    plt.title('B-factor predictions for protein 1V70', fontsize=16)
